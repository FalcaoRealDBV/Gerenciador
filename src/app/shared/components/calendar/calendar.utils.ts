...
export function getDayAriaLabel(day: CalendarDay): string {
  ...
  const labels = [
    dateStr,
    day.isToday && 'Hoje',
    day.isSelected && 'Selecionado',
    day.isRangeStart && 'Início do intervalo',
    day.isRangeEnd && 'Fim do intervalo',
    day.isInRange && 'Dentro do intervalo',
    !day.isCurrentMonth && 'Fora do mês',
    day.isDisabled && 'Indisponível',
  ].filter(Boolean);
